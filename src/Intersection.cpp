...
std::vector<std::shared_ptr<Street>> Intersection::queryStreets(
  ...
  for (auto it : _streets) {
    if (incoming->ID() != it->ID())  // ... except the street making the inquiry
    {
...
void Intersection::addVehicleToQueue(std::shared_ptr<Vehicle> vehicle) {
  ...
  lck.lock();
  std::cout << "Intersection #" << _id << ": Vehicle #" << vehicle->ID()
...
void Intersection::processVehicleQueue() {
  ...
  while (true) {
    ...
    if (_waitingVehicles.Size() > 0 && !_isBlocked) {
...
bool Intersection::trafficLightIsGreen() {
  ...
  if (_traffic_light.CurrentPhase() == TrafficLightPhase::green)
    return true;
  else
    return false;
